...
def _add_heading(doc: Document, text: str):
    ...
    _set_paragraph_spacing(paragraph, space_before=6, space_after=2)
    _add_horizontal_line(paragraph)


def _add_bullets(doc: Document, bullets: Iterable[str], font_size: int = 9):
    for bullet in bullets:
        paragraph = doc.add_paragraph(style="List Bullet")
        run = paragraph.add_run(bullet)
        _set_run_font(run, size=font_size)
        _set_paragraph_spacing(paragraph, space_before=0, space_after=1)


def _setup_document(doc: Document):
    """Set up slim margins and remove headers/footers."""
...
def build_docx(cv: dict, output_path: str) -> None:
    doc = Document()
    _setup_document(doc)
    ...
    _set_paragraph_spacing(name_para, space_before=0, space_after=0)
    ...
    if title:
        ...
        _set_run_font(title_run, size=10)
    ...
    if contact_parts:
        ...
        _set_run_font(contact_run, size=8)
    ...
    if summary:
        ...
        _set_run_font(summary_run, size=9)
    ...
    if experience:
        ...
        for item in experience:
            ...
            _set_run_font(header_run, size=9, bold=True)
            _set_paragraph_spacing(header_para, space_before=3, space_after=1)
            _add_bullets(doc, item.get("bullets", []), font_size=9)
    ...
    if projects:
        ...
        for item in projects:
            ...
            _set_run_font(name_run, size=9, bold=True)
    ...
    if education:
        ...
        for item in education:
            ...
            if details:
                _add_bullets(doc, details, font_size=9)
    ...
    if skills:
        ...
        for group in skills:
            ...
            _set_run_font(group_run, size=9, bold=True)
            items_run = group_para.add_run(items)
            _set_run_font(items_run, size=9)
